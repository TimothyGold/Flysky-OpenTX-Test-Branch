...
#define MAX_CURVES \
  16  // TODO next EEPROM check if can be changed to 32 to have all ARM the same
#define MAX_CURVE_POINTS 512
#else
#define LEN_MODEL_NAME 10
#define LEN_FLIGHT_MODE_NAME 6
...
#elif LCD_W >= 212
enum MainViews { VIEW_TIMERS, VIEW_INPUTS, VIEW_SWITCHES, VIEW_COUNT };
#else
enum MainViews {
  VIEW_OUTPUTS_VALUES,
  VIEW_OUTPUTS_BARS,
...
enum BeeperMode { e_mode_quiet = -2, e_mode_alarms, e_mode_nokeys, e_mode_all };
...
#elif defined(PCBSKY9X)
enum ModuleIndex { EXTERNAL_MODULE, EXTRA_MODULE, TRAINER_MODULE };
#endif
...
#else
#define TRAINER_MODE_MAX()                                                  \
  HAS_WIRELESS_TRAINER_HARDWARE() ? TRAINER_MODE_MASTER_BATTERY_COMPARTMENT \
                                  : TRAINER_MODE_MASTER_CPPM_EXTERNAL_MODULE
#endif
...
#define TELEMETRY_SCREEN_TYPE(screenIndex) \
  TelemetryScreenType((g_model.frsky.screensType >> (2 * (screenIndex))) & 0x03)
#define IS_BARS_SCREEN(screenIndex) \
  (TELEMETRY_SCREEN_TYPE(screenIndex) == TELEMETRY_SCREEN_TYPE_GAUGES)
#else
...
enum PotsWarnMode { POTS_WARN_OFF, POTS_WARN_MANUAL, POTS_WARN_AUTO };
...
// even we do not spend space in EEPROM for 10 GVARS, we reserve the space
// inside the range of values, like offset, weight, etc.
...
enum SwitchSources {
  ...
  SWSRC_LAST_MULTIPOS_SWITCH =
      SWSRC_FIRST_MULTIPOS_SWITCH + (NUM_XPOTS * XPOTS_MULTIPOS_COUNT) - 1,
#endif
  ...
  SWSRC_LAST_LOGICAL_SWITCH =
      SWSRC_FIRST_LOGICAL_SWITCH + MAX_LOGICAL_SWITCHES - 1,
...
enum MixSources {
  ...
  MIXSRC_FIRST_INPUT,
  LUA_EXPORT_MULTIPLE("input", "Input [I%d]", MAX_INPUTS) MIXSRC_LAST_INPUT =
      MIXSRC_FIRST_INPUT + MAX_INPUTS - 1,
#endif
  ...
  LUA_EXPORT("rud", "Rudder") MIXSRC_Ele,
  LUA_EXPORT("ele", "Elevator") MIXSRC_Thr,
  LUA_EXPORT("thr", "Throttle") MIXSRC_Ail,
  ...
  LUA_EXPORT("s1", "Potentiometer S1") MIXSRC_6POS,
  LUA_EXPORT("6pos", "Multipos Switch") MIXSRC_S2,
  LUA_EXPORT("s2", "Potentiometer S2") MIXSRC_FIRST_SLIDER,
  MIXSRC_S3 = MIXSRC_FIRST_SLIDER,
  LUA_EXPORT("s3", "Slider S3") MIXSRC_S4,
  LUA_EXPORT("s4", "Slider S4") MIXSRC_LS,
  LUA_EXPORT("ls", "Left rear slider") MIXSRC_RS,
  LUA_EXPORT("rs", "Right rear slider") MIXSRC_LAST_POT = MIXSRC_RS,
#elif defined(PCBX9E)
  MIXSRC_POT1 = MIXSRC_FIRST_POT,
  LUA_EXPORT("s1", "Potentiometer 1") MIXSRC_POT2,
  LUA_EXPORT("s2", "Potentiometer 2") MIXSRC_POT3,
  LUA_EXPORT("s3", "Potentiometer 3") MIXSRC_POT4,
  LUA_EXPORT("s4", "Potentiometer 4 (X9E only)") MIXSRC_FIRST_SLIDER,
  MIXSRC_SLIDER1 = MIXSRC_FIRST_SLIDER,
  LUA_EXPORT("ls", "Left slider") MIXSRC_SLIDER2,
  LUA_EXPORT("rs", "Right slider") MIXSRC_SLIDER3,
  LUA_EXPORT("lcs", "Left center slider (X9E only)") MIXSRC_SLIDER4,
  LUA_EXPORT("rcs", "Right center slider (X9E only)") MIXSRC_LAST_POT =
      MIXSRC_SLIDER4,
#elif defined(PCBX7) || defined(PCBXLITE) || defined(PCBNV14)
  MIXSRC_POT1 = MIXSRC_FIRST_POT,
  LUA_EXPORT("s1", "Potentiometer 1") MIXSRC_POT2,
  LUA_EXPORT("s2", "Potentiometer 2") MIXSRC_LAST_POT = MIXSRC_POT2,
#elif defined(PCBTARANIS)
  MIXSRC_POT1 = MIXSRC_FIRST_POT,
  LUA_EXPORT("s1", "Potentiometer 1") MIXSRC_POT2,
  LUA_EXPORT("s2", "Potentiometer 2") MIXSRC_POT3,
  LUA_EXPORT("s3", "Potentiometer 3") MIXSRC_FIRST_SLIDER,
  MIXSRC_SLIDER1 = MIXSRC_FIRST_SLIDER,
  LUA_EXPORT("ls", "Left slider") MIXSRC_SLIDER2,
  LUA_EXPORT("rs", "Right slider") MIXSRC_LAST_POT = MIXSRC_SLIDER2,
#else
...
#if defined(PCBHORUS)
  MIXSRC_MOUSE1,
  LUA_EXPORT("jsx", "Joystick X") MIXSRC_MOUSE2,
  ...
  LUA_EXPORT("cyc1", "Cyclic 1") MIXSRC_CYC2,
  LUA_EXPORT("cyc2", "Cyclic 2") MIXSRC_CYC3,
  LUA_EXPORT("cyc3", "Cyclic 3") MIXSRC_LAST_HELI = MIXSRC_CYC3,

  MIXSRC_FIRST_TRIM,
  MIXSRC_TrimRud = MIXSRC_FIRST_TRIM,
  LUA_EXPORT("trim-rud", "Rudder trim") MIXSRC_TrimEle,
  LUA_EXPORT("trim-ele", "Elevator trim") MIXSRC_TrimThr,
  LUA_EXPORT("trim-thr", "Throttle trim") MIXSRC_TrimAil,
  LUA_EXPORT("trim-ail", "Aileron trim")
#if defined(PCBHORUS)
      MIXSRC_TrimT5,
  LUA_EXPORT("trim-t5", "Aux trim T5") MIXSRC_TrimT6,
  LUA_EXPORT("trim-t6", "Aux trim T6") MIXSRC_LAST_TRIM = MIXSRC_TrimT6,
#else
  ...
  LUA_EXPORT("sa", "Switch A") MIXSRC_SB,
  LUA_EXPORT("sb", "Switch B") MIXSRC_SC,
  LUA_EXPORT("sc", "Switch C") MIXSRC_SD,
  LUA_EXPORT("sd", "Switch D") MIXSRC_SE,
  LUA_EXPORT("se", "Switch E") MIXSRC_SF,
  LUA_EXPORT("sf", "Switch F") MIXSRC_SG,
  LUA_EXPORT("sg", "Switch G") MIXSRC_SH,
  LUA_EXPORT("sh", "Switch H") MIXSRC_LAST_SWITCH = MIXSRC_SH,
#elif defined(PCBXLITE)
  MIXSRC_SA = MIXSRC_FIRST_SWITCH,
  LUA_EXPORT("sa", "Switch A") MIXSRC_SB,
  LUA_EXPORT("sb", "Switch B") MIXSRC_SC,
  LUA_EXPORT("sc", "Switch C") MIXSRC_SD,
  LUA_EXPORT("sd", "Switch D") MIXSRC_LAST_SWITCH = MIXSRC_SD,
...
#if defined(PCBX9E)
      MIXSRC_SI,
  LUA_EXPORT("si", "Switch I") MIXSRC_SJ,
  LUA_EXPORT("sj", "Switch J") MIXSRC_SK,
  LUA_EXPORT("sk", "Switch K") MIXSRC_SL,
  LUA_EXPORT("sl", "Switch L") MIXSRC_SM,
  LUA_EXPORT("sm", "Switch M") MIXSRC_SN,
  LUA_EXPORT("sn", "Switch N") MIXSRC_SO,
  LUA_EXPORT("so", "Switch O") MIXSRC_SP,
  LUA_EXPORT("sp", "Switch P") MIXSRC_SQ,
  LUA_EXPORT("sq", "Switch Q") MIXSRC_SR,
  LUA_EXPORT("sr", "Switch R") MIXSRC_LAST_SWITCH = MIXSRC_SR,
#else
  ...
  LUA_EXPORT_MULTIPLE("ls", "Logical switch L%d", MAX_LOGICAL_SWITCHES)
      MIXSRC_LAST_LOGICAL_SWITCH =
          MIXSRC_FIRST_LOGICAL_SWITCH + MAX_LOGICAL_SWITCHES - 1,
  ...
  LUA_EXPORT_MULTIPLE("ch", "Channel CH%d", MAX_OUTPUT_CHANNELS) MIXSRC_CH2,
  MIXSRC_CH3,
  MIXSRC_CH4,
  MIXSRC_CH5,
  MIXSRC_CH6,
  ...
  MIXSRC_LAST_RESERVE,
  MIXSRC_FIRST_TIMER,
  MIXSRC_TIMER1 = MIXSRC_FIRST_TIMER,
  LUA_EXPORT("timer1", "Timer 1 value [seconds]") MIXSRC_TIMER2,
  LUA_EXPORT("timer2", "Timer 2 value [seconds]") MIXSRC_TIMER3,
  LUA_EXPORT("timer3", "Timer 3 value [seconds]") MIXSRC_LAST_TIMER =
      MIXSRC_TIMER3,
#endif
...
enum TimerModes { TMRMODE_SIMPLE, TMRMODE_THR, TMRMODE_THR_REL, TMRMODE_COUNT };
...
enum ResetFunctionParam {
  ...
  FUNC_RESET_PARAM_LAST_TELEM =
      FUNC_RESET_PARAM_FIRST_TELEM + MAX_TELEMETRY_SENSORS,
#endif
