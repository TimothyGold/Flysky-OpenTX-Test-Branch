...
/* new TextButton(window, {LCD_W/2-125, window->height() - 150, 250, 30}, STR_QUICK_START_GUIDE, [=]() -> int8_t { new QR_CodeMenu (QR_CODE_QUICK_START); 
    return 1;
    });
    
    new TextButton(window, {LCD_W/2-125, window->height() - 150, 250, 30}, STR_USER_MANUAL, [=]() -> int8_t { new QR_CodeMenu (QR_CODE_USER_MANUAL);
    return 1;
    });
*/
