...
void onSourceLongEnterPress(const char *result) {
  if (result == STR_MENU_INPUTS)
    ...
    checkIncDecSelection =
        getFirstAvailable(MIXSRC_FIRST_CH, MIXSRC_LAST_CH, isSourceAvailable);
...
// #define DBLKEYS_PRESSED_RGT_LFT(in)    ((in & ((1<<KEY_RIGHT) +
// (1<<KEY_LEFT))) == ((1<<KEY_RIGHT) + (1<<KEY_LEFT))) #define
// DBLKEYS_PRESSED_UP_DWN(in)     ((in & ((1<<KEY_UP) + (1<<KEY_DOWN))) ==
// ((1<<KEY_UP) + (1<<KEY_DOWN))) #define DBLKEYS_PRESSED_RGT_UP(in)     ((in &
// ((1<<KEY_RIGHT) + (1<<KEY_UP))) == ((1<<KEY_RIGHT) + (1<<KEY_UP))) #define
// DBLKEYS_PRESSED_LFT_DWN(in)    ((in & ((1<<KEY_LEFT) + (1<<KEY_DOWN))) ==
// ((1<<KEY_LEFT) + (1<<KEY_DOWN)))
...
#define HORTAB(row) \
  (int8_t(row) > (int8_t)horTabMax ? horTab[horTabMax] : horTab[row])
...
bool check(event_t event, uint8_t curr, const MenuHandlerFunc *menuTab,
           uint8_t menuTabSize, const pm_uint8_t *horTab, uint8_t horTabMax,
           int rowcount, uint8_t flags) {
  return true;
}

bool check_simple(event_t event, uint8_t curr, const MenuHandlerFunc *menuTab,
                  uint8_t menuTabSize, int rowcount) {
