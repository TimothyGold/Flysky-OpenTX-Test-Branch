...
class DefaultTheme : public Theme {
 public:
  DefaultTheme() : Theme("Default", OPTIONS_THEME_DEFAULT) { loadColors(); }
  ...
  void loadMenuIcon(uint8_t index, const char* filename,
                    uint32_t color = MENU_TITLE_COLOR) const {
    ...
    if (mask) {
      ...
      menuIconNormal[index] = new BitmapBuffer(BMP_RGB565, TOPBAR_BUTTON_WIDTH,
                                               TOPBAR_BUTTON_WIDTH);
      if (menuIconNormal[index]) {
        menuIconNormal[index]->clear(HEADER_BGCOLOR);
        menuIconNormal[index]->drawMask((TOPBAR_BUTTON_WIDTH - width) / 2,
                                        (TOPBAR_BUTTON_WIDTH - width) / 2, mask,
                                        color);
      }

      // TODO delete menuIconSelected[index];
      menuIconSelected[index] = new BitmapBuffer(
          BMP_RGB565, TOPBAR_BUTTON_WIDTH, TOPBAR_BUTTON_WIDTH);
      if (menuIconSelected[index]) {
        ...
        menuIconSelected[index]->drawMask((TOPBAR_BUTTON_WIDTH - width) / 2,
                                          (TOPBAR_BUTTON_WIDTH - height) / 2,
                                          mask, color);
      }
    }
  }
  ...
  void loadIcons() const {
    ...
    loadMenuIcon(ICON_RADIO_GLOBAL_FUNCTIONS,
                 "mask_radio_global_functions.png");
    ...
    loadMenuIcon(ICON_MODEL_LOGICAL_SWITCHES,
                 "mask_model_logical_switches.png");
    loadMenuIcon(ICON_MODEL_SPECIAL_FUNCTIONS,
                 "mask_model_special_functions.png");
    ...
    BitmapBuffer* background =
        BitmapBuffer::loadMask(getThemePath("mask_currentmenu_bg.png"));
    BitmapBuffer* shadow =
        BitmapBuffer::loadMask(getThemePath("mask_currentmenu_shadow.png"));
    BitmapBuffer* dot =
        BitmapBuffer::loadMask(getThemePath("mask_currentmenu_dot.png"));
    ...
    if (currentMenuBackground) {
      currentMenuBackground->drawSolidFilledRect(
          0, 0, currentMenuBackground->getWidth(), MENU_HEADER_HEIGHT,
          HEADER_BGCOLOR);
      currentMenuBackground->drawSolidFilledRect(
          0, MENU_HEADER_HEIGHT, currentMenuBackground->getWidth(),
          MENU_TITLE_TOP - MENU_HEADER_HEIGHT, TEXT_BGCOLOR);
      currentMenuBackground->drawSolidFilledRect(
          0, MENU_TITLE_TOP, currentMenuBackground->getWidth(),
          currentMenuBackground->getHeight() - MENU_TITLE_TOP, TITLE_BGCOLOR);
    ...
    topleftBitmap = BitmapBuffer::loadMaskOnBackground(
        "topleft.png", TITLE_BGCOLOR, HEADER_BGCOLOR);
  ...
  void loadThemeBitmaps() const {
    ...
    calibStickBackground =
        BitmapBuffer::load(getThemePath("stick_background.png"));

    // TODO delete calibTrackpBackground;
    calibTrackpBackground =
        BitmapBuffer::load(getThemePath("trackp_background.png"));
    ...
    modelselIconBitmap = BitmapBuffer::loadMaskOnBackground(
        "modelsel/mask_iconback.png", TITLE_BGCOLOR, TEXT_BGCOLOR);
    if (modelselIconBitmap) {
      BitmapBuffer* bitmap =
          BitmapBuffer::load(getThemePath("modelsel/icon_default.png"));
      modelselIconBitmap->drawBitmap(20, 8, bitmap);
      // TODO delete bitmap;
    }

    // TODO delete modelselSdFreeBitmap;
    modelselSdFreeBitmap = BitmapBuffer::loadMaskOnBackground(
        "modelsel/mask_sdfree.png", TEXT_COLOR, TEXT_BGCOLOR);

    // TODO delete modelselModelQtyBitmap;
    modelselModelQtyBitmap = BitmapBuffer::loadMaskOnBackground(
        "modelsel/mask_modelqty.png", TEXT_COLOR, TEXT_BGCOLOR);

    // TODO delete modelselModelNameBitmap;
    modelselModelNameBitmap = BitmapBuffer::loadMaskOnBackground(
        "modelsel/mask_modelname.png", TEXT_COLOR, TEXT_BGCOLOR);

    // TODO delete modelselModelMoveBackground;
    modelselModelMoveBackground =
        BitmapBuffer::loadMask(getThemePath("modelsel/mask_moveback.png"));

    // TODO delete modelselModelMoveIcon;
    modelselModelMoveIcon =
        BitmapBuffer::loadMask(getThemePath("modelsel/mask_moveico.png"));

    // TODO delete modelselWizardBackground;
    modelselWizardBackground =
        BitmapBuffer::load(getThemePath("wizard/background.png"));

    // Channels monitor screen
    // TODO delete chanMonLockedBitmap;
    chanMonLockedBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_monitor_lockch.png", TEXT_COLOR, TEXT_BGCOLOR);

    // TODO delete chanMonInvertedBitmap;
    chanMonInvertedBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_monitor_inver.png", TEXT_COLOR, TEXT_BGCOLOR);

    // Mixer setup screen
    // TODO delete mixerSetupMixerBitmap;
    mixerSetupMixerBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_sbar_mixer.png", MENU_TITLE_COLOR, HEADER_BGCOLOR);

    // TODO delete mixerSetupToBitmap;
    mixerSetupToBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_sbar_to.png", MENU_TITLE_COLOR, HEADER_BGCOLOR);

    // TODO delete mixerSetupOutputBitmap;
    mixerSetupOutputBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_sbar_output.png", MENU_TITLE_COLOR, HEADER_BGCOLOR);

    // TODO delete mixerSetupAddBitmap;
    mixerSetupAddBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_mplex_add.png", TEXT_COLOR, TEXT_BGCOLOR);

    // TODO delete mixerSetupMultiBitmap;
    mixerSetupMultiBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_mplex_multi.png", TEXT_COLOR, TEXT_BGCOLOR);

    // TODO delete mixerSetupReplaceBitmap;
    mixerSetupReplaceBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_mplex_replace.png", TEXT_COLOR, TEXT_BGCOLOR);

    // TODO delete mixerSetupLabelBitmap;
    mixerSetupLabelBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_textline_label.png", TEXT_COLOR, TEXT_BGCOLOR);

    // TODO delete mixerSetupCurveBitmap;
    mixerSetupCurveBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_textline_curve.png", TEXT_COLOR, TEXT_BGCOLOR);

    // TODO delete mixerSetupSwitchBitmap;
    mixerSetupSwitchBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_textline_switch.png", TEXT_COLOR, TEXT_BGCOLOR);

    // TODO delete mixerSetupFlightmodeBitmap;
    mixerSetupFlightmodeBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_textline_fm.png", TEXT_COLOR, TEXT_BGCOLOR);

    // TODO delete mixerSetupSlowBitmap;
    mixerSetupSlowBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_textline_slow.png", TEXT_COLOR, TEXT_BGCOLOR);

    // TODO delete mixerSetupDelayBitmap;
    mixerSetupDelayBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_textline_delay.png", TEXT_COLOR, TEXT_BGCOLOR);

    // TODO delete mixerSetupDelaySlowBitmap;
    mixerSetupDelaySlowBitmap = BitmapBuffer::loadMaskOnBackground(
        "mask_textline_delayslow.png", TEXT_COLOR, TEXT_BGCOLOR);
  }
  ...
  virtual void updatecolor() const {
    ...
    uint32_t bg_color = UNEXPECTED_SHUTDOWN()
                            ? WHITE
                            : g_eeGeneral.themeData.options[0].unsignedValue;
...
#define DARKER(x) ((x * 70) / 100)
    lcdColorTable[HEADER_BGCOLOR_INDEX] =
        RGB(DARKER(GET_RED(color)), DARKER(GET_GREEN(color)),
            DARKER(GET_BLUE(color)));
  ...
  virtual void drawBackground() const {
    if (!backgroundBitmap || backgroundBitmap->getFormat() == BMP_ARGB4444 ||
        backgroundBitmap->getWidth() != LCD_W ||
        backgroundBitmap->getHeight() != LCD_H) {
  ...
  virtual bool drawTopbarBackground(uint32_t options) const {
    ...
    if (options & OPTION_TOPBAR_LOGO) {
      ...
      lcd->drawSolidFilledRect(TOPBAR_BUTTON_WIDTH, 0,
                               LCD_W - TOPBAR_BUTTON_WIDTH, MENU_HEADER_HEIGHT,
                               HEADER_BGCOLOR);
    } else {
  ...
  virtual void drawIcon(coord_t x, coord_t y, uint8_t index,
                        LcdFlags flags) const {
    lcd->drawMask(x, y, iconMask[index], flags);
  }

  virtual const BitmapBuffer* getIconBitmap(uint8_t index,
                                            bool selected) const {
