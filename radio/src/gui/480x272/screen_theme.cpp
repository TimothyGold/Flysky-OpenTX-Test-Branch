...
bool ZoneOptionPage::isChangeAllowed(const ZoneOption* option) { return true; }

void ZoneOptionPage::onZoneOptionChanged(const ZoneOption* option) {}

void ZoneOptionPage::addOption(Window* window, GridLayout& grid,
                               const ZoneOption& option,
...
Window* ZoneOptionPage::createOptionEdit(Window* parent, const rect_t& rect,
                                         const ZoneOption* option,
                                         ZoneOptionValue* value) {
  switch (option->type) {
    ...
    case ZoneOption::Integer:
      return new NumberEdit(parent, rect, option->min.signedValue,
    ...
    case ZoneOption::String:
      return new TextEdit(parent, rect, value->stringValue,
                          sizeof(value->stringValue));
    case ZoneOption::File:
      return new FileChoice(
          parent, rect, BITMAPS_PATH, BITMAPS_EXT, sizeof(value->stringValue),
          [=]() {
            return std::string(value->stringValue, ZLEN(value->stringValue));
          },
          [=](std::string newValue) {
            if (!isChangeAllowed(option)) return;
            strncpy(value->stringValue, newValue.c_str(),
                    sizeof(value->stringValue));
            onZoneOptionChanged(option);
          },
          false);
    case ZoneOption::TextSize:
      return new Choice(parent, rect,
                        "\010StandardTiny\0   Small\0  Mid\0    Double", 0, 4,
    ...
    case ZoneOption::Timer: {
      auto timerchoice =
          new Choice(parent, rect, nullptr, 0, TIMERS - 1,
                     [=] { return (int16_t)value->unsignedValue; },
                     [=](int16_t newValue) {
                       if (!isChangeAllowed(option)) return;
                       value->unsignedValue = newValue;
    ...
    case ZoneOption::Color:
      new ColorEdit(parent, rect, [=] { return value->signedValue; },
                    [=](int32_t newValue) {
                      if (!isChangeAllowed(option)) return;
                      value->signedValue = newValue;
                      onZoneOptionChanged(option);
...
void ScreenThemePage::build(Window* window) {
      ...
      }, [=](int16_t newValue)
      {
        ...
        for (auto it = themes.begin(); it != themes.end(); ++it, index++)
        {
          if(index == newValue) {
            (*it)->init();
            loadTheme((*it));
            strncpy(g_eeGeneral.themeName, (*it)->getName(),
  sizeof(g_eeGeneral.themeName)); storageDirty(EE_GENERAL); window->clear();
            build(window);
          }
        }
      });
  ...
  */
  ...
  new TextButton(window, grid.getLineSlot(), STR_TOP_BAR, []() -> uint8_t {
    new WidgetsSetupView(static_cast<WidgetsContainerInterface*>(topbar), 0,
                         MENU_TITLE_COLOR, 2, 1);
    return 0;
  });
  grid.nextLine();
