...
class Layout : public WidgetsContainer<MAX_LAYOUT_ZONES, MAX_LAYOUT_OPTIONS> {
 ...
 public:
  ...
  unsigned int getZonesCount() const override { return zonesCount; }

  inline const LayoutFactory* getFactory() const { return factory; }
  ...
  uint16_t flightModeHeight() const { return isOptionSet(FlightMode) ? 10 : 0; }

  uint16_t trimHeight() const { return isOptionSet(Trims) ? 16 : 0; }

  uint16_t sliderHeight() const { return isOptionSet(Sliders) ? 16 : 0; }
 ...
 protected:
  uint16_t margin() const { return 10; }
...
class LayoutFactory {
 public:
  LayoutFactory(const char* name) : name(name) { registerLayout(this); }
...
class BaseLayoutFactory : public LayoutFactory {
 public:
  BaseLayoutFactory(const char* name, const uint8_t* bitmap,
                    const ZoneOption* options)
      : LayoutFactory(name), bitmap(bitmap), options(options) {}

  void drawThumb(BitmapBuffer* dc, uint16_t x, uint16_t y,
                 uint32_t flags) const override {
    dc->drawBitmapPattern(x, y, bitmap, flags);
  }

  virtual const ZoneOption* getOptions() const { return options; }
