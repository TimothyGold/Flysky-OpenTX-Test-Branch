...
void CurveEdit::update() {
  ...
  for (uint8_t i = 0; i < 5 + curve.points; i++) {
    if (hasFocus() && current == i) {
      position = [=]() -> int { return getPoint(index, i).x; };
    } else {
...
bool CurveEdit::onTouchEnd(coord_t x, coord_t y) {
  ...
  for (int i = 0; i < 5 + curve.points; i++) {
    if (i != current) {
      ...
      if (abs(getPointX(point.x) - x) <= 10 &&
          abs(getPointY(point.y) - y) <= 10) {
        current = i;
        pointSelected = true;
        update();
        break;
...
void CurveEdit::onFocusLost() { CurveKeyboard::instance()->disable(true); }
...
void CurveEdit::right() {
  CurveInfo &curve = g_model.curves[index];
  if (curve.type == CURVE_TYPE_CUSTOM && current != 0 &&
      current != curve.points - 1) {
