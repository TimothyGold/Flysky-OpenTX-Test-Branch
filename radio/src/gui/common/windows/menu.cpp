...
MenuWindow::MenuWindow(Menu* parent)
    : Window(parent,
             {LCD_W / 2 - 100,
              LCD_H / 2 - 30 /* to avoid redraw the menus header */, 200, 0},
             OPAQUE) {
...
bool MenuWindow::onTouchEnd(coord_t x, coord_t y) {
  ...
  lines[index].onPress();
  return false;  // = close the menu (inverted so that click outside the menu
                 // closes it)
}
...
void MenuWindow::paint(BitmapBuffer* dc) {
  ...
  for (unsigned i = 0; i < lines.size(); i++) {
    dc->drawText(10, i * lineHeight + (lineHeight - 22) / 2,
                 lines[i].text.data(),
                 selectedIndex == (int)i ? WARNING_COLOR : MENU_TITLE_COLOR);
    if (i > 0) {
...
void Menu::updatePosition() {
  if (!toolbar) {
    // there is no navigation bar at the left, we may center the window on
    // screen
  ...
  menuWindow.setInnerHeight(menuWindow.lines.size() *
                            menuWindow.getLineHeight());
}
...
bool Menu::onTouchSlide(coord_t x, coord_t y, coord_t startX, coord_t startY,
